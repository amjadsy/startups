...
**Never merge per-service product attribution into one sentence.** "Lambda
qualifies for a Compute Savings Plan" and "DynamoDB on-demand qualifies for a
Database Savings Plan" are two separate facts about two separate products —
Lambda has no Database-SP path and DynamoDB has no Compute-SP path. State each
service's eligible product by name.
