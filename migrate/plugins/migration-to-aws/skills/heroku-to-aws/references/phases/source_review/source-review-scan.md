...
binding fields, including reachability beyond loopback. Do not infer optional values
such as conventional service ports. Treat executable or application files, not
configuration files passed to them, as process entrypoints. Inspect lockfiles for
platform-specific native packages. A logging facade is not a telemetry destination.
Exclude scripts that only provision or deploy the current Heroku test environment from
application `release_setup_commands`.
