...
2. **`awspricing` MCP (secondary)** — If a model is NOT in `pricing-cache.md` and MCP is
   available, query `get_pricing("AmazonBedrock", ...)` with the model filter and the user's
   target region. Set `pricing_source: "live"`.
3. **Cache after MCP failure (`cached_fallback`)** — If MCP was attempted but failed, and the
   model IS in the cache, use the cached price. Set `pricing_source: "cached_fallback"`.
4. **Unavailable** — If a model is NOT in the cache AND MCP failed, consult
   `shared/pricing-fallback.md` (tertiary), set `pricing_source: "unavailable"`, emit `null`
   for the affected per-token fields, and warn the user. Never fabricate a token price.

For typical migrations (Claude, Llama, Nova, Mistral, DeepSeek, Gemma, OpenAI gpt-oss, and the
proprietary GPT-5.x source rows), ALL prices are in `pricing-cache.md`. Zero MCP calls needed.
...
proprietary GPT-5.x models, so the `awspricing` MCP returns no rows for them. An empty result is
**not** evidence the model is unavailable or free. Use `shared/pricing-cache.md`, and treat rows
