...
If `cpu_architecture` is `x86` (user opted out) or no compute targets arm64, skip this subsection.
...
- **Migration shape**: Extract from `generation-*.json` `migration_plan.duration_drivers` (approach + ordered stages; operational windows like watch periods may be stated as procedure — never staffing estimates or week/hour figures)
