...
9. **compute.tf modification (runs during Step 3 compute domain, not here):** every `aws_launch_template` emitted for ECS-EC2, **standard EKS (`eks-standard`) node groups**, or bare EC2 receives IMDSv2 enforcement unconditionally — the security baseline is always applied. **EKS Auto Mode (`eks-auto`) emits no launch template** — AWS provisions Auto Mode nodes on Bottlerocket with IMDSv2 enforced by default, so there is nothing to modify here (do not synthesize a launch template for Auto Mode):
...
- **CPU architecture (Graviton/ARM64):** read each compute resource's
  ...
  `aws_config` (e.g., `m7g.xlarge`) on EC2 / **standard EKS** (`eks-standard`) launch templates, and for Elastic Beanstalk the
  ...
  EKS arm64 node groups are single-arch on dev tier. **EKS Auto Mode (`eks-auto`) — Graviton is NOT a Terraform flag.** The built-in `general-purpose` NodePool is **amd64-only and immutable**, and `aws_eks_cluster` cannot express node architecture, so arm64 app workloads require a **custom Karpenter NodePool + EC2NodeClass Kubernetes manifest** applied post-provision (see the EKS branch in Step 3.1). When `graviton.target_architecture == arm64` on an Auto Mode cluster, emit that manifest (`k8s/nodepool-arm64.yaml`) plus a `kubectl apply` step in the runbook; if the run cannot emit/apply Kubernetes manifests, set the cluster to **amd64**, skip the arm64 image build, and flag Graviton as not applied — do **not** report arm64.
...
| Compute    | Emit Fargate/EKS/ECR per the skill's posture (private subnets, EKS private endpoint, ECR scan-on-push). Populate task CPU/memory and autoscaling from `aws_config`. Apply Graviton/ARM64 wiring from the CPU architecture rule above. **EKS branch — select the cluster type from the resource's resolved `aws_service` in `aws-design.json` (`EKS Auto Mode`, `Standard EKS Cluster`, or `Fargate`), not from `design_constraints.kubernetes`** — the latter is the global Q8 preference and can differ from this resource's target when the Auto Mode eliminator changed it in Design. For a resource resolved to **EKS Auto Mode** (`eks-auto`, the default for GKE sources on the **AWS-only** path) emit an **EKS Auto Mode** cluster — `aws_eks_cluster` with `compute_config { enabled = true, node_pools = ["general-purpose"] }` (plus `system`), `kubernetes_network_config { elastic_load_balancing { enabled = true } }`, `storage_config { block_storage { enabled = true } }`, and the Auto Mode cluster IAM role + the node IAM role its `compute_config.node_role_arn` references; do **not** emit `aws_eks_node_group` or `aws_launch_template` (Auto Mode provisions and manages nodes via Karpenter/Bottlerocket — no user-managed node groups). **Graviton on Auto Mode is not a Terraform flag:** the built-in `general-purpose` NodePool is amd64-only and immutable and `aws_eks_cluster` cannot set node architecture. When `graviton.target_architecture == arm64`, emit an additional **Kubernetes manifest** `k8s/nodepool-arm64.yaml` — a custom Karpenter `NodePool` (`requirements: [{key: kubernetes.io/arch, operator: In, values: ["arm64"]}]`) plus a custom `EC2NodeClass` (the built-in `default` NodeClass is tied to the built-in pools) — and add a `kubectl apply -f k8s/nodepool-arm64.yaml` step to the post-provision runbook, with a note that arm64 pods must also carry a matching `nodeSelector`/affinity. If the run does not produce/apply Kubernetes manifests (Terraform-only output), keep the cluster on **amd64** and record `graviton` as not applied in the design rationale — never report arm64 for a cluster that only has the built-in amd64 `general-purpose` pool. For **`eks-standard`** — **including the `compute: "eks"` multi-cloud override, which always uses standard node groups (Auto Mode is AWS-specific and not used for multi-cloud)** — emit `aws_eks_node_group` (managed node groups) with the Graviton instance type from `aws_config` and a launch template (subject to the IMDSv2 rule in item 9). For **`ecs-fargate`** (or non-GKE containers) emit Fargate as before. **Elastic Beanstalk** (App Engine → EB): emit one `aws_elastic_beanstalk_application` for the app and one `aws_elastic_beanstalk_environment` per `source_service` (from `aws_config`). Resolve `solution_stack_name` with a **Terraform `data` source** (self-updating, needs no generate-time credentials, cannot go stale in the committed file) — do not paste `aws_config.platform` verbatim (it is a human-readable label like `"Python 3.12 running on 64bit Amazon Linux 2023"`): emit `data "aws_elastic_beanstalk_solution_stack" "<svc>" { most_recent = true, name_regex = "64bit Amazon Linux 2023 (.*) running Python 3.12" }` (build `name_regex` from the language + version in `platform`) and reference `.name`. Fallback only if a `data` source can't be used: `aws elasticbeanstalk list-available-solution-stacks` at generate time. Also emit the EB **instance profile** this environment references — `aws_iam_role` (EC2 trust) + `aws_iam_instance_profile` + the managed-policy attachment (`AWSElasticBeanstalkWebTier`, plus `AWSElasticBeanstalkWorkerTier`/`AWSElasticBeanstalkMulticontainerDocker` as the platform needs) — since EB does not auto-create it in Terraform; do not reference an instance profile that no resource emits. Emit `setting` blocks for: `aws:autoscaling:launchconfiguration/IamInstanceProfile` (the instance profile just emitted); `aws:autoscaling:launchconfiguration/SecurityGroups` (an SG for the instances, required when they sit in private subnets); `aws:autoscaling:launchconfiguration/InstanceType` (from `instance_type`); `aws:elasticbeanstalk:environment/EnvironmentType` (from `environment_type`); **when `environment_type == "LoadBalanced"`, also `aws:elasticbeanstalk:environment/LoadBalancerType = application`** (ALB, not the Classic default) plus `aws:autoscaling:asg/MinSize` (from `min_instances`) and `MaxSize` (from `max_instances`); `aws:ec2:vpc/VPCId`, `Subnets`, and `ELBSubnets` — for **LoadBalanced**, instances in private subnets + ALB in public (`ELBScheme` public); for **SingleInstance** (no ALB) put the instance in a **public** subnet with `AssociatePublicIpAddress=true` so it is reachable; and `aws:elasticbeanstalk:application:environment` for env vars. ARM64: pick the `t4g.*` instance type from `aws_config` per the Graviton rule. See `design-refs/elastic-beanstalk.md`. |
