...
| GKE             | EKS               | Custom CRI incompatible → manual workaround or ECS. **Auto Mode caveat:** node-level needs Auto Mode disallows (custom AMIs/OS, node-managing privileged DaemonSets, SSH/host access, custom CRI or kubelet flags) → use a Standard EKS Cluster (`kubernetes: "eks-standard"`), not Auto Mode.                                              |
...
| App Engine      | Elastic Beanstalk | `compute_model: "container_orchestration"` or `"serverless"` in preferences → do not use EB, fall through to Fargate or Lambda _(preference override, not a technical blocker)_. Also `compute: "eks"` (Q5 = multi-cloud) → do not use EB, route to a **Standard EKS Cluster** (portable; not Auto Mode) _(top-level portability override)_ |
...
- **Auto Mode regional availability:** no separate regional gate is required. EKS Auto Mode tracks the EKS service footprint — it is available in every current EKS region (all commercial, GovCloud, China, and EU Sovereign Cloud partitions) — so `eks-auto` deploys anywhere EKS itself is available. This differs from Bedrock model selection (`references/phases/design/design-ai.md`), where models have genuine per-region variance that warrants the `aws___get_regional_availability` check. Revisit only if a future region ships EKS before Auto Mode.

### App Engine

- **Multi-cloud portability required** (`compute: "eks"` from Q5) → **Standard EKS Cluster** (`kubernetes: "eks-standard"`) — top-level portability override; takes precedence over the EB default and any `compute_model` preference (Q7b does not fire under multi-cloud). Same override that forces GKE → Standard EKS Cluster. **Auto Mode is NOT used for multi-cloud** — its node management is AWS-specific and defeats the portability that Q5 requires; a Standard EKS Cluster is the portable target.
...
After selecting Elastic Beanstalk, load `elastic-beanstalk.md` to populate `aws_config` (platform, deployment policy, IAM, VPC, sizing). When `compute: "eks"` routes App Engine to a Standard EKS Cluster under multi-cloud, do **not** run the EB fan-out or load `elastic-beanstalk.md` — the app_version resources are skipped like any other non-EB path.
...
1. **Eliminators**: Does GCP config violate AWS constraints, or does a top-level preference override apply? If yes: switch to alternative. **`compute: "eks"` (Q5 = multi-cloud) is a hard override — App Engine (and all compute) → Standard EKS Cluster (portable Kubernetes; Auto Mode is NOT used for multi-cloud); stop here, do not evaluate the managed-platform/EB branch below.** For GKE (AWS-only, Auto Mode default), the Auto Mode eliminator applies: node-level customization Auto Mode disallows → `eks-standard`, not Fargate.
2. **Operational Model**: Managed (Lambda, Fargate, **EKS Auto Mode** — AWS operates the nodes) vs Self-Hosted (EC2, **Standard EKS Cluster**)?
   - Prefer managed unless: Always-on + high baseline cost → EC2, or the user explicitly wants node control → `eks-standard`
   - For App Engine sources: Elastic Beanstalk (PaaS-to-PaaS) when `compute_model` is absent or `"managed_platform"` **and `compute` ≠ `"eks"`** (when `compute: "eks"`, the criterion-1 override already selected a Standard EKS Cluster)
3. **User Preference**: From `preferences.json`: `design_constraints.compute`, `design_constraints.kubernetes`, `design_constraints.cost_sensitivity`?
   - If `compute = "eks"` (Q5 = multi-cloud) → **Standard EKS Cluster** for all compute, including App Engine (top-level portability override; overrides EB default and `compute_model`). Auto Mode is excluded here — it is AWS-specific and defeats multi-cloud portability.
   - If `kubernetes = "eks-auto"` → **EKS Auto Mode** (keep Kubernetes, AWS runs the nodes)
   - If `kubernetes = "eks-standard"` → **Standard EKS Cluster with managed node groups** (user explicitly wants node control)
...
- Criterion 3 (User Preference): `compute = "eks"` → Standard EKS Cluster (multi-cloud portability override; Auto Mode excluded — AWS-specific, not portable)
- → **AWS: Standard EKS Cluster** (no EB fan-out; app_version resources skipped like any other non-EB path)
- Confidence: `inferred` (rubric-based override of default PaaS mapping)
...
- Criterion 1 (Eliminators): Auto Mode caveat fires → fall back to a Standard EKS Cluster with managed node groups (not Fargate)
- → **AWS: Standard EKS Cluster with managed node groups** — Kubernetes preserved with the node control the workload requires
- Confidence: `inferred`
- Rationale: `"GKE → Standard EKS Cluster (Auto Mode incompatible: node-level customization required)"`
