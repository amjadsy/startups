...
| `design-refs/elastic-beanstalk.md`               | `google_app_engine_application` in inventory (optionally with `compute_model == "managed_platform"` in preferences). Supplementary reference for EB configuration detail (platforms, IAM, VPC, deployment policies). Does not replace `compute.md` — both may be needed in mixed projects. Skip when no App Engine is in inventory, even if `compute_model` is set. |
