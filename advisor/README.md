...
- **`gcp-to-aws`** — Plan a migration from Google Cloud Platform — and OpenAI/Gemini AI workloads — to AWS, directly in your IDE. Runs a guided, multi-phase workflow: discover resources from Terraform/IaC, app code, and GCP billing exports (optionally supplemented with real AI spend from the OpenAI Admin API — read-only, consent-gated, aggregate counts only); design an AWS architecture; estimate costs; and generate migration artifacts. OpenAI workloads land on the **same GPT model on Amazon Bedrock** where one exists (including OpenAI models reached through OpenRouter or LiteLLM), with a cross-family swap to Claude/Nova as the alternative; Gemini usage maps to closest-fit Bedrock model families. GCP Document AI, Vision, and Speech-to-Text calls are detected and routed to AWS traditional-AI services. On the infrastructure-generation route (Terraform/IaC discovered), Generate also emits `baseline.tf`, an account security baseline with the same always-on controls as `heroku-to-aws`. Config and Security Hub are added only when root `preferences.json.compliance` contains soc2, pci, hipaa, or fedramp; Clarify stores the Q2 answer at `design_constraints.compliance.value` and does not set that root field, so a standard Clarify compliance answer does not include those controls. AI-only runs instead emit `bedrock_monitoring.tf`, and billing-only runs emit skeleton Terraform — neither includes the account security baseline.
- **`azure-to-aws`** — Plan a migration from Microsoft Azure — and Azure OpenAI / agentic AI workloads — to AWS. Runs a 6-phase workflow: discover resources from Terraform (`azurerm_*`), app code, and Azure billing exports; design an AWS architecture; estimate costs (1:1 lift and right-sized); optionally reprice what-if scenarios; and generate migration artifacts when the user opts in. App Service → Elastic Beanstalk, AKS → EKS, Cosmos DB → DynamoDB, Azure OpenAI → Bedrock. (Bicep, ARM template, and live `az` CLI discovery are planned follow-ups, not yet available — a Bicep/ARM/live-only workspace halts rather than guessing.)
- **`heroku-to-aws`** — Plan a migration from Heroku to AWS (Dynos → Elastic Beanstalk by default; Fargate/EKS overrides, Postgres → RDS/Aurora, Redis → ElastiCache, Kafka → MSK, plus 13+ common add-ons), with an optional what-if repricing workshop after the estimate. Generate also emits `baseline.tf`, an account security baseline (CloudTrail, GuardDuty, IAM password policy, S3 account public-access block, EBS default encryption, IAM Access Analyzer, IMDS defaults, alternate contacts, and a budget — plus Config and Security Hub when `global.compliance` includes soc2, pci, hipaa, or fedramp) alongside the app Terraform.
...
**Commitment discounts (Savings Plans / Reserved Instances).** Both migration estimates check every AWS target against a shared RI/SP eligibility matrix and present commitment options in a dedicated report section. What gets a dollar figure depends on the input: when Design has mapped a target instance class — from Terraform/IaC or from live discovery (e.g. Cloud SQL `settings.tier`) — RDS/Aurora Reserved Instances and Database Savings Plans are sized from it (prod-tier databases above $50/month on-demand); Compute Savings Plans (Fargate/Lambda/EC2) are always shown as a percentage range, never sized from source-cloud billing — the skills recommend 30–90 days of AWS on-demand usage first, then AWS Cost Explorer's recommendations. Billing-only runs have no instance sizes and do not produce the dedicated RI/SP section: they surface commitment guidance only when the billing export shows active commitments (CUDs), as a conditional GCP-CUD-vs-AWS percentage comparison — a no-CUD billing-only run gets neither the dedicated section nor percentage options.
...
- **`prompt-library-for-startups`** — AWS-curated copy-paste prompts plus downloadable installable agents (Multi-Account Transition Advisor, Bill Shock Preventer, Service Quota Agent, Bedrock Model Availability Agent, AWS DB Advisor).

The skills are designed to be cross-aware — `start-building-for-startups` consults `knowledge-base-for-startups` and `prompt-library-for-startups` mid-flow; `knowledge-base-for-startups` and `prompt-library-for-startups` defer to each other on boundary queries; and migration intent routes to the matching migration skill (`gcp-to-aws`, `azure-to-aws`, `heroku-to-aws`, or `llm-to-bedrock`) or to `agent-advisor` for agent runtime selection.

## MCP servers

The migration skills (`gcp-to-aws`, `azure-to-aws`, `heroku-to-aws`, `llm-to-bedrock`, `agent-advisor`) use MCP servers, declared in `advisor/plugins/aws-startup-advisor/.mcp.json`, for live AWS data. The skills run without them — pricing falls back to a bundled cache (±5-25% accuracy) and AWS documentation lookups are skipped — but current AWS docs need them configured:

- **AWS MCP Server** (`aws-mcp`, HTTP) — AWS documentation, regional availability, and architecture guidance (the unified AWS MCP Server). Unlike the previous zero-auth knowledge endpoint, connecting may require a one-time AWS sign-in (OAuth 2.1) or SigV4 via a local proxy (`mcp-proxy-for-aws`); the doc/regional tools themselves need no IAM permissions.
...
**The `npx skills add` path below does not configure MCP servers.** The `npx skills` CLI only copies skill files into your agent's skills folder — it has no concept of `.mcp.json` and never touches MCP configuration. If you install this way (Kiro, Cursor, Codex, GitHub Copilot, and everything else in the supported-agent list below), the skills still work per the fallback behavior above, but you'll get current AWS docs only after adding the servers yourself:
...
| _"Migrate my Azure app to AWS"_                                 | `azure-to-aws` → 6-phase migration workflow                                                                     |
...
npx skills remove azure-to-aws --agent <agent>
