...
This supersedes the unconditional statement in Step 4b: include an AgentCore I/O-wait billing advantage only if `answers.system.current_run_verifications["agentcore.io_wait_billing"]` is verified in this run. Without that evidence, say only that the billing behavior requires current verification and do not make a comparative billing claim.
