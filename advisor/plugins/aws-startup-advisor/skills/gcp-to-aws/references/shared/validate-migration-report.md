...
`advisor/plugins/aws-startup-advisor/fixtures/migration-report-reference.html` — TOC `href` values match `section id` attributes exactly, security detail lives in the appendix behind a teaser, mapping rationale is in `<details>` blocks, and headings carry no numbers. Validate with the full-contract command above.
